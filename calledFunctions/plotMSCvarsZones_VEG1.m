...
function [sname, T_corr, sp] = plotMSCvarsZones_VEG1(dataNames, dataObs, data1, data2, zoneNames, zoneIdx, pix_a, unitNames, col)
...
for rr=1:nrows
    ...
    for cc=1:ncols
        ...
        T_rmse0.(varN){1,cc} = round(corr(tmpO(:),tmp1(:), 'rows', 'complete'),2);
        T_rmse0.(varN){2,cc} = round(corr(tmpO(:),tmp2(:), 'rows', 'complete'),2);

        
        %add the correlation in the plot
        if strcmp(varN, 'wTWS')==1
            if cc==5
                t1 = text(0.97,0.175, ['r^2 = ' num2str(T_corr.(varN){1,cc})],'Units','Normalized', 'Fontsize', 8, 'Color', col(2,:), 'HorizontalAlignment', 'right');
                t2 = text(0.97,0.075, ['r^2 = ' num2str(T_corr.(varN){2,cc})],'Units','Normalized', 'Fontsize', 8, 'Color', col(3,:), 'HorizontalAlignment', 'right');
            else
                t1 = text(0.03,0.175, ['r^2 = ' num2str(T_corr.(varN){1,cc})],'Units','Normalized', 'Fontsize', 8, 'Color', col(2,:), 'HorizontalAlignment', 'left');
                t2 = text(0.03,0.075, ['r^2 = ' num2str(T_corr.(varN){2,cc})],'Units','Normalized', 'Fontsize', 8, 'Color', col(3,:), 'HorizontalAlignment', 'left');
            end
        elseif strcmp(varN, 'ET')==1
            if cc==2 || cc==3
                t1 = text(0.97,0.825, ['r^2 = ' num2str(T_corr.(varN){1,cc})],'Units','Normalized', 'Fontsize', 8, 'Color', col(2,:), 'HorizontalAlignment', 'right');
                t2 = text(0.97,0.925, ['r^2 = ' num2str(T_corr.(varN){2,cc})],'Units','Normalized', 'Fontsize', 8, 'Color', col(3,:), 'HorizontalAlignment', 'right');
            else
                t1 = text(0.97,0.175, ['r^2 = ' num2str(T_corr.(varN){1,cc})],'Units','Normalized', 'Fontsize', 8, 'Color', col(2,:), 'HorizontalAlignment', 'right');
                t2 = text(0.97,0.075, ['r^2 = ' num2str(T_corr.(varN){2,cc})],'Units','Normalized', 'Fontsize', 8, 'Color', col(3,:), 'HorizontalAlignment', 'right');
            end
...
l  = legend(tmpT, 'Position', [.44 -.02 .12 .07], 'box', 'off', 'Orientation', 'Horizontal','Fontsize', 10);
