%% do the comparisons of VEG Paper additional with the PFT run
%% set save pth
spth = [pwd '/plots/PFT_vs_B_vs_VEG/'];
...
% %% load forcing
% f_in = 'data/input/globalBaseline_Constraints_1deg.mat'
...
% colors
if strcmp(space_run,'opti904')==1
    colLabel.size   = 5;
else
    colLabel = [];
end

% get the cluster zones
...
%      print([spth 'considered Regions.png'],'-dpng','-r300')
...
[sname, T_corr,  T_AIC, T_BIC, sp] = plotMSCvarsZones3(dataNames, dataObsMSC, data1MSC, data2MSC, data3MSC, n_paramsOpti, zoneNames, KG_v, pix_a, unitNames, []);
print(gcf,[spth_p sname '_withAIC.png'],'-dpng','-r300');



%% Plot the Correlation Maps
% B vs VEG
plotDiffMaps2(dataNames(1:3), dataObs, data1, data2, lat, lon, [], spth_p)
close all
plotDiffMaps2(dataNames(1:3), dataObsMSC, data1MSC, data2MSC, lat, lon, [], spth_p)

% B vs PFT
plotDiffMaps2(dataNames([1,2,4]), dataObs, data1, data3, lat, lon, [], spth_p)
close all
plotDiffMaps2(dataNames([1,2,4]), dataObsMSC, data1MSC, data3MSC, lat, lon, [], spth_p)

% VEG vs PFT
plotDiffMaps2(dataNames([1,3,4]), dataObs, data2, data3, lat, lon, [], spth_p)
close all
plotDiffMaps2(dataNames([1,3,4]), dataObsMSC, data2MSC, data3MSC, lat, lon, [], spth_p)
...
%% as Impact Index (Getirana et al. 2017) - all storages seperate
...
for eN=1:numel(expNames)
    ...
    for ii=1:numel(aggTimes)
        ...
        % test if C_wTotal = sum of other storages
        testDiff = (C_wSnow+C_wSoil+C_wGW+C_wSurf) - C_wTotal
...
% Plot the IAV for zones -with wSnow
for eN=1:numel(expNames)
    expN = expNames{eN};
