%% set save pth
spth = [pwd '/plots/VEG_vs_noGW2Soil/'];
if ~exist(spth, 'dir'), mkdir(spth),end

%% load model output
expNames    = {'E_B_bL_RD4', 'E_B_bL_RD4_noGW2Soil'}; 
rDates      = {'20200629', '20201012'};
...
% get the zones
pth_CLregions = 'data/input/clusterRegions.mat';
load(pth_CLregions, 'CLregions');
...
%% compare with noGW2wSoil
for eN=1:numel(expNames)
    expN = expNames{eN};  
    % overall difference in ET (grid-wise mean)
    dataIn.(expN).evapTotal_mean = mean(mod.(expN).evapTotal,2);
end


%Comparison Maps - mean of ET - % difference
varNames_comp = {'evapTotal_mean'}
plotDiffMaps_InputPercDiff(expNames, varNames_comp, dataIn, lat, lon, colLabel, spth)
